"""指数历史采集：中证全指(000985)、成长100(980080)、红利低波(H30269)。

数据源优先级（按当前环境验证可用性排列）：
- 000985 / H30269：中证指数官网 API（官方源，含成交额）→ 东财行情（本机）→ 腾讯K线
- 980080：东财行情（本机）→ 腾讯成长ETF(159259) 代理 K线
...
INDEX_SOURCES: dict[str, list[tuple[str, callable]]] = {
    ...
    "index:980080:close": [
        ("em", _from_em_index("0.980080")),
        ("tx", _from_tx_kline("sz159259")),  # 成长ETF易方达（跟踪中证成长100），ETF 代理
    ],
}
